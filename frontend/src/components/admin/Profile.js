...
const Profile = () => {
  ...
  useEffect(() => {
    axios.get(API_BASE_URL + "players/get_player/" + getLoginInfo("admin", "user_id")).then(res => {
  ...
  useEffect(() => {
    ...
    axios.post(API_BASE_URL + "players/update_profile_photo/" + getLoginInfo("admin", "user_id"), formData).then(res => {
      if (res.data.status == 1) {
        toast.success(res.data.message);
        setPreview(BASE_URL + "profile_photo/" + res.data.response.image);
        updateLoginInfo("admin", "profile", res.data.response.image);
      } else {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post(API_BASE_URL + "players/update_player_info/" + getLoginInfo("admin", "user_id"), formValues).then(res => {
      setIsSubmitting(false);
      if (res.data.status == 1) {
        updateLoginInfo("admin", "is_completed", 1);
        toast.success(res.data.message);
      } else {
        toast.error(res.data.message);
      }
