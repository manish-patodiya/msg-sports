...
const Profile = ({ updateComponent }) => {
  ...
  useEffect(() => {
    axios.get(API_BASE_URL + "players/get_player/" + getLoginInfo("player", "user_id")).then(res => {
  ...
  useEffect(() => {
    ...
    axios.post(API_BASE_URL + "players/update_profile_photo/" + getLoginInfo("player", "user_id"), formData).then(res => {
      if (res.data.status == 1) {
